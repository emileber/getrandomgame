...
docks::docks(char** map, point* p) : zoneType()
{
    map[p->Getx()][p->Gety()]='d';
}
    ...
    while(!found)
    {
        ...
        while(++j<3&&!found)
        {
            ...
            if(newX>=0&&newX<mapsize)
            {
                ...
                if(map[newX][possible->at(i)->Gety()]!='e'&&map[newX][possible->at(i)->Gety()]!='k'&&map[newX][possible->at(i)->Gety()]!='Q')
                {
                     found=true;
        ...
        while(++j<3&&!found)
        {
           ...
           if(newY>=0&&newY<mapsize)
            {
                if(map[possible->at(i)->Getx()][newY]!='e'&&map[possible->at(i)->Getx()][newY]!='Q'&&map[possible->at(i)->Getx()][newY]!='k')
                {
                     found=true;
